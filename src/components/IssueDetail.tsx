...
export function IssueDetail({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ProgressTimeline steps={ISSUE_STEPS} labelMap={ISSUE_STATUS_LABEL} current={issue.status} />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[2fr_1fr]">
        <div className="flex flex-col gap-6">
          <div className="grid grid-cols-2 gap-4 rounded-lg border border-black/10 p-4 text-sm sm:grid-cols-4 dark:border-white/10">
            ...
            <Field label="크루">{issue.assignee?.name ?? "미배정"}</Field>
          </div>
          ...
          <div className="rounded-lg border border-black/10 p-4 dark:border-white/10">
            ...
            <p className="mt-3 text-sm text-foreground/80">
              {room.room_number}호에서 접수된{" "}
              {ISSUE_CATEGORY_LABEL[issue.category]} 이슈입니다. 게스트가
              &ldquo;{issue.description}&rdquo;라고 신고했으며, 현재{" "}
            ...
            {(issue.ai_suggested_category || issue.ai_suggested_urgency) && (
              <p className="mt-3 text-xs text-gray-500 dark:text-gray-400">
                ...
                {issue.ai_suggested_category
                  ...
                  : "-"}{" "}
                / 긴급도 {issue.ai_suggested_urgency ?? "-"} — 운영자가 위
                값으로 최종 확정했습니다.
              </p>
            )}
          </div>
